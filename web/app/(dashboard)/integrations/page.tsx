...
import {
  ...
  ChevronRight, CircleOff, ExternalLink, Filter, KeyRound, Loader2, Plug, Plus, RefreshCw,
...
/**
 * Re-authorizes an account in place: same connection id, so agent grants, tool
 * policies and history survive. Nothing to disconnect first.
 */
function reconnectUrl(connection: Connection) {
  const base = connection.connector.authMode === 'oauth-google' ? '/api/auth/google' : '/api/mcp/oauth/start';
  return `${base}?connectionId=${connection.id}`;
}

function needsReconnect(connection: Connection) {
  return connection.status === 'reauth_required' || connection.status === 'error'
    || missingConnectionScopes(connection.connector.scopes, connection.grantedScopes).length > 0;
}

function ConnectionPanel({ connection, agents, secretFields, onClose, refresh }: { connection: Connection; agents: Agent[]; secretFields: Connector['secretFields']; onClose: () => void; refresh: () => void }) {
  ...
  const isSecret = connection.connector.authMode === 'secret';
  const enabledCount = connection.discoveredTools.length - disabledTools.length;

  async function reconnect() {
    if (!isSecret) { window.location.assign(reconnectUrl(connection)); return; }
    if (!secretPrompt) { setSecretPrompt(true); return; }
    setBusy(true); setReconnectError('');
    const response = await fetch(`/api/connections/${connection.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ credentials: secrets }) });
    setBusy(false);
    if (!response.ok) { setReconnectError((await response.json().catch(() => ({}))).error || 'Reconnexion impossible'); return; }
    setSecretPrompt(false); setSecrets({}); refresh();
  }
...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    ...
    if (params.get('mcp') === 'reconnected') {
      setBanner({ kind: 'ok', text: 'Compte reconnecté. Les agents, outils et politiques ont été conservés.' });
      setTab('connected'); refreshConnections(); refreshCatalog(); window.history.replaceState({}, '', '/integrations');
    }
    else if (params.get('mcp') === 'connected') {
